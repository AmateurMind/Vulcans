        ...
        <div className="p-8">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-[var(--muted-foreground)] text-sm">{certs?.length ?? 0} certificates issued</p>
                </div>
                <button
                            ...
                            {[
                                ...
                                <div key={f.label}>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-2.5 rounded-xl bg-[var(--card)] border border-[var(--border)] text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:border-yellow-500/40 text-sm transition-all"
                                    />
                                </div>
                            ))}

                            <div>
                                <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-1.5">Certificate File (PDF/Image, optional)</label>
                                <button
                                    type="button"
                                    onClick={() => fileRef.current?.click()}
                                    className="w-full h-20 border border-dashed border-[var(--border)] rounded-xl flex items-center justify-center gap-2 text-[var(--muted-foreground)] hover:text-yellow-400 hover:border-yellow-500/30 transition-all text-sm"
                                >
            ...
            <div className="flex flex-col gap-3">
                {certs === undefined && <p className="text-[var(--muted-foreground)] text-sm">Loading...</p>}
                {certs?.length === 0 && (
                    <div className="glass rounded-2xl p-12 text-center text-[var(--muted-foreground)]">
...
function CertRow({ cert, onRemove }: { cert: { _id: Id<"certificates">; title: string; recipientName: string; year: string; fileId?: Id<"_storage"> }; onRemove: () => void }) {
    ...
    return (
        <div className="glass rounded-xl px-5 py-4 flex items-center gap-4 hover:border-[var(--primary)]/30 transition-all">
            <div className="p-2 rounded-lg bg-yellow-500/10 shrink-0">
                <Award className="w-5 h-5 text-yellow-400" />
            </div>
            <div className="flex-1 min-w-0">
                <p className="font-semibold text-[var(--foreground)] text-sm truncate">{cert.title}</p>
                <p className="text-[var(--muted-foreground)] text-xs">{cert.recipientName} · {cert.year}</p>
            </div>
            {fileUrl && (
                <a href={fileUrl} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg text-[var(--muted-foreground)] hover:text-blue-400 hover:bg-blue-500/10 transition-all shrink-0">
                    <ExternalLink className="w-4 h-4" />
                </a>
            )}
            <button onClick={onRemove} className="p-2 rounded-lg text-[var(--muted-foreground)] hover:text-red-400 hover:bg-red-500/10 transition-all shrink-0">
